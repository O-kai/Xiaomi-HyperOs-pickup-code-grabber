...
## [2.9.2] - 2026-09-13 · 规则引擎评分体系进化与正反向过滤机制

- **🛑 正反向规则体系与评分熔断机制**：
  - 针对“商品在代收点已超6小时未取，为避免生鲜损坏请尽快取件”等催取/滞留/超时提醒短信，引入正反向双通道判定；
  - 细化评分系统：在候选扫描层，结合时长（小时/天/分）、前置修饰（超/过/满）以及整体负向词库（已超/超时/滞留/损坏等）进行严密特征扣分与安全熔断，彻底杜绝单字数字“6”被误认为取件码的严重逻辑 bug；
- **⚡ 规则集热更新平滑兼容 v3**：
  - 更新发布 rules.json v3 规则集，无缝向下兼容，规则拉取支持端侧本地冒烟自愈校验；
- **📋 疑似漏抓错题本交互优化**：
  - 无论错题本是否为空，均提供极简直达的「✍️ 手动上报」入口，一键唤起浏览器自动带出预填模板，免去繁琐说明；
- **🧪 语料回归集扩充与全量绿灯**：
  - 增加催取反例与特殊短信回归用例，27 条取件码与 13 条地点用例 100% 全量自动化测试通过。
